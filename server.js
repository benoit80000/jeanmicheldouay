...
function makeLocalBusinessSchema(baseUrl) {
  return {
    '@context': 'https://schema.org',
    '@type': 'LocalBusiness',
    name: site.organization.name,
    url: baseUrl,
    image: `${baseUrl}${site.organization.logo}`,
...
function page(res, req, pageConfig) {
  const canonicalUrl = `${site.baseUrl}${req.path === '/' ? '/' : req.path}`;
  const schemas = [makeLocalBusinessSchema(site.baseUrl)];
...
for (const pageConfig of pages) {
  app.get(pageConfig.path, (req, res) => page(res, req, pageConfig));
}
...
app.get('/sitemap.xml', (req, res) => {
  ...
  const urls = pages
    .map((p) => `<url><loc>${site.baseUrl}${p.path}</loc><lastmod>${now}</lastmod><changefreq>weekly</changefreq></url>`)
    .join('');
...
app.use((req, res) => {
  res.status(404).render('pages/404', {
    siteName: site.siteName,
    title: 'Page introuvable – JMD Maçonnerie',
